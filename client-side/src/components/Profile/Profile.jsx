...
const Profile = ({setUser,profile,setProfile}) => {
    ...
    const getPosts=async()=>{
        const res=await axios.get("http://localhost:3001/api/getpost",{headers:{"Authorization":`Bearer ${token}`}})
        console.log(res);
        setPost(res.data);
      }
    const deleteUser=async(_id)=>{
       if(confirm("Do you want to delete your account?")){
        const res= await axios.delete(`http://localhost:3001/api/deleteuser/${_id}`)
        // console.log(res);
                ...
                <tbody>
                    ...
                    <tr>
                        <td className="actions" colSpan="2">
                          <Link to={'/editprofile'}>
                           <button type="button" className="edit-btn">{proBool?'EDIT':'CREATE'}</button></Link>
